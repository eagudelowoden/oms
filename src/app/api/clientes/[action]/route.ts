...
import { ClienteBackendService } from "@/modules/auth/services/cliente.service";
  ...
  if (action === "switch") {
    try {
      ...
      if (!dbNameReal) {
        ...
        return NextResponse.json(
          { error: "Nombre de base de datos técnica no encontrado" },
          { status: 400 },
        );
      }
      ...
      console.log(
        `🔄 Realizando switch a DB Original: ${dbName} (${clienteNombre})`,
      );
      ...
      return NextResponse.json(
        { error: "No se pudo procesar el cambio de cliente" },
        { status: 500 },
      );
    }
  }
