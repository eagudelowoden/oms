...
function SortIcon({ col, sortCol, sortAsc }: { col: string; sortCol: string | null; sortAsc: boolean }) {
...
export default function PrealertaTabla({
  isLoading, items, query, setQuery,
  sortCol, sortAsc, onSort,
  seleccionada, onSeleccionar, onEliminar,
}: Props) {
  return (
    <div className={styles.card}>
      ...
      <div className={styles.tableWrap}>
        <table className={styles.table}>
          <thead>
            <tr>
              <th onClick={() => onSort("nombre")} className={styles.thSortable}>
                NOMBRE <SortIcon col="nombre" sortCol={sortCol} sortAsc={sortAsc}/>
              </th>
              <th onClick={() => onSort("fecha")} className={styles.thSortable}>
                FECHA <SortIcon col="fecha" sortCol={sortCol} sortAsc={sortAsc}/>
              </th>
              <th>ESTADO</th>
              <th>ACCIONES</th>
            </tr>
          ...
          <tbody>
            {isLoading ? (
              ...
              items.map((item, i) => (
                <tr
                  key={i}
                  className={seleccionada?.nombre === item.nombre ? styles.trSelected : ""}
