...
const WFSM_LOGIN_URL_S = process.env.WFSM_LOGIN_URL_S!;
const WFSM_CONSULTA_URL_SERIALES = process.env.WFSM_CONSULTA_URL_SERIALES!;
const WFSM_AUTH_BASIC_S = process.env.WFSM_AUTH_BASIC_S!;
  ...
  const sincronizarDesdeAPI = async (
    ...
    try {
      ...
      const consultaRes = await fetch(
        `${WFSM_CONSULTA_URL_SERIALES}?${params.toString()}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
