...
import InfoAutorizacionPopup, {
  InfoAutorizacion,
} from "@/app/admin/components/InfoAutorizacionPopup";

interface Props {
  prealertas: PrealertaEnvioRow[];
  isLoading: boolean;
  seleccionada: PrealertaEnvioRow | null;
  onSeleccionar: (p: PrealertaEnvioRow) => void;
  onRefresh: () => void;
...
export default function TablaPrealertas({
  ...
  const handleAbrirPopup = (e: React.MouseEvent, p: PrealertaEnvioRow) => {
    e.stopPropagation();
    setPopupInfo({
      id: p.id,
      noAutorizacion: p.noAutorizacion,
      observacion: p.observacion,
      creadoPor: p.usuarioNombre,
    });
  };
                    ...
                    <td>
                      {p.noAutorizacion ? (
                        <button
                          ...
                          onClick={(e) => handleAbrirPopup(e, p)}
                          title="Ver detalles de autorización"
                        >
                          {p.noAutorizacion}
                        </button>
                      ) : (
